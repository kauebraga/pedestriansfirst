...
ghsl_countries <- distinct(ghsl_2024_filter1, region, country1) %>%
...
# to download
url_download_africa <- ghsl_countries %>% filter(region == "south-america")
response_download <- purrr::map2(url_download_africa$url,
                        url_download_africa$file_dir,
                        # sprintf("input_data/pbf/%s_202411.osm.pbf", url_download_africa$country), 
                        possibly(curl::curl_download, quiet = TRUE, otherwise = "erro")
                      )
names(response) <- ghsl_countries$country[1:10]
  

  
# export
ghsl_2024_filter1 %>% fwrite("input_data/ghsl_regions.csv")
