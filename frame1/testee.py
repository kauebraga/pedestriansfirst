...
def process_bike(edges, quilt_allbike, quilt_protectedbike, center_nodes):
  ...
  edges = edges.drop(
    columns=['name', 'width', 'oneway', 'reversed', 'length', 'lanes', 
             'ref', 'maxspeed', 'access', 'bridge', 'service', 'junction', 'area']
  )
...
bike_conditions = (
  (edges['highway'] == 'cycleway') |
    (edges['highway'] == 'path') & (edges['bicycle'] == 'designated') |
    (edges['cycleway:right'].isin(['track', 'lane', 'opposite_track'])) |
    (edges['cycleway:left'].isin(['track', 'lane', 'opposite_track'])) |
    (edges['cycleway:both'].isin(['track', 'lane', 'opposite_track'])) |
    (edges['cycleway'].isin(['track', 'lane', 'opposite_track']))
)
...
total_allbike['is_protected'] = (
  (total_allbike['highway'] == 'cycleway') |
    ((total_allbike['highway'] == 'path') & (total_allbike['bicycle'] == 'designated')) |
    (total_allbike['cycleway:right'].isin(['track', 'opposite_track'])) |
    (total_allbike['cycleway:left'].isin(['track', 'opposite_track'])) |
    (total_allbike['cycleway:both'].isin(['track', 'opposite_track'])) |
    (total_allbike['cycleway'].isin(['track', 'opposite_track']))
)
...
center_nodes['pnpb'] = set(total_protectedbike.index.map(lambda x: x[0]).tolist() + 
                             total_protectedbike.index.map(lambda x: x[1]).tolist())
center_nodes['pnab'] = set(total_allbike.index.map(lambda x: x[0]).tolist() + 
                             total_allbike.index.map(lambda x: x[1]).tolist())
